(function(w, idbName){
    ...
    var DB = function(name, initialTable, callback){
        "use strict"; // 使用严格模式
        this.name = name;
        if(this.name === undefined){
            throw 'name is empty!';
        }
        this.initialTable = initialTable ? initialTable : {};
    ...
    DB.prototype.open = function(resultData) {
        if (indexedDB !== null) {
 ...
 */
var DB1 = function(name, initialTable) {
    "use strict";
    var self = this;
        ...
        if (self.type === 'DB') {
            ...
            table.addAll = function(arrayData, resultData) {
                ...
                for(var i=0; i<total; i++){
                    objectStore.add(arrayData[i]).onsuccess = callfun;
                }
                return self;
            };
            /**
            ...
            table.saveAll = function(arrayData, resultData) {
                ...
                if(total === 0){
                    ...
                    for(var i=0; i<total; i++){
                        objectStore.put(arrayData[i]).onsuccess = callfun;
                    }
                }
                return self;
            };
